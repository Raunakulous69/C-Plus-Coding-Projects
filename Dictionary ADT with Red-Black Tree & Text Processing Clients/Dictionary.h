...
private:
   ...
   // pair in tree R, arranged in order by keys.
   void inOrderString(std::string& s, Node* R) const;

   // preOrderString()
   // Appends a string representation of the tree rooted at R to s. The 
   // appended string will consist of keys only, with the order determined 
   // by a pre-order tree walk. The keys stored in black Nodes will be 
   // appended as "key\n", and the keys stored in red Nodes will be appended 
...
public:
   ...
   // pre_string()
   // Returns a string consisting of all keys in this Dictionary. The key 
   // order is given by a pre-order tree walk. The keys stored in black 
   // Nodes will appear in the returned string as "key\n", and the keys 
   // stored in red Nodes will appear as "key (RED)\n".  The output of this 
   // function is demonstrated in /Examples/pa8/DictionaryClient-out.
